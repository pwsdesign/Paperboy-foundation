...
export default function StoryPage({
  ...
  return (
    <main className="wrap py-16 md:py-24">
      {/* Back */}
      <div className="mb-6">
        <Link href="/stories" className="underline text-[13px]">
          ← Back to Stories
        </Link>
      </div>
      ...
      <section className="rulebox p-7 md:p-12 relative overflow-hidden">
        {/* Optional grain overlay if you keep .grain in globals */}
        <div className="grain">
          ...
          <section key={idx} className={idx === 0 ? "" : "mt-10"}>
            ...
            <div className="mt-4 space-y-5">
              {sec.body.map((p, i) => (
                <p
                  key={i}
                  className="lede text-[15px] md:text-[16px] leading-relaxed"
                >
                  {p}
                </p>
              ))}
