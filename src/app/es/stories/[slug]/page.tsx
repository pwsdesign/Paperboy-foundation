...
import { getStoryES, STORIES_ES } from "@/lib/stories.es";

export function generateStaticParams() {
  return STORIES_ES.map((s) => ({ slug: s.slug }));
}

export default function StoryPage({ params }: { params: { slug: string } }) {
  const lang = "es";
  const story = getStoryES(params.slug);
    ...
    <main className="wrap py-16 md:py-24">
        ...
        <div className="grain">
          ...
          {story.coverQuote ? (
            <div className="mt-8 border-t pt-6" style={{ borderColor: "var(--rule)" }}>
              <p className="kicker">Una línea del texto</p>
      ...
      <article className="mt-10 zine-card p-7 md:p-12">
        ...
        <div className="mt-12 border-t pt-6" style={{ borderColor: "var(--rule)" }}>
          <p className="kicker">Nota impresa</p>
          <p className="lede mt-3 text-[14px] leading-relaxed">
            Esta historia aparece en el fanzine impreso como un feature visual. La versión web existe para leer más a fondo — no para scrollear.
          </p>

          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <Link href={`/${lang}/issue-01`} className="btn-outline">Ver Edición 01</Link>
            <Link href={`/${lang}/support`} className="btn-ink">Apoyar la impresión</Link>
          </div>
        </div>
      </article>
    </main>
