...
export function generateStaticParams() {
  // This is okay even inside [lang] route
    ...
    <main className="wrap py-16 md:py-24">
      <div className="mb-6">
        <Link href={`/${lang}/stories`} className="underline text-[13px]">
          {lang === "es" ? "← Volver a Historias" : "← Back to Stories"}
        </Link>
      </div>
      ...
      <section className="rulebox p-7 md:p-12 relative overflow-hidden">
        <div className="grain">
          ...
          <h1 className="hero-statement mt-5 text-4xl md:text-6xl">
            {story.title}
          </h1>
          ...
          {story.coverQuote ? (
            <div className="mt-8 border-t pt-6" style={{ borderColor: "var(--rule)" }}>
              <p className="kicker">{lang === "es" ? "Una línea" : "A line from the piece"}</p>
              <p className="lede mt-3 text-[16px] leading-relaxed">
                “{story.coverQuote}”
              </p>
            </div>
          ) : null}
      ...
      <article className="mt-10 zine-card p-7 md:p-12">
        {story.sections.map((sec, idx) => (
          <section key={idx} className={idx === 0 ? "" : "mt-10"}>
            ...
            <div className="mt-4 space-y-5">
              {sec.body.map((p, i) => (
                <p key={i} className="lede text-[15px] md:text-[16px] leading-relaxed">
                  {p}
                </p>
              ))}
            </div>
        ...
        <div className="mt-12 border-t pt-6" style={{ borderColor: "var(--rule)" }}>
          <p className="kicker">{lang === "es" ? "Nota impresa" : "Print note"}</p>
          <p className="lede mt-3 text-[14px] leading-relaxed">
            {lang === "es"
              ? "Esta historia aparece en el fanzine impreso como una pieza visual. La versión web existe para leer con calma."
              : "This story appears in the print fanzine as a visual-first feature. The web version exists for deeper reading, not scrolling."}
          </p>

          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <Link href={`/${lang}/issue-01`} className="btn-outline">
              {lang === "es" ? "Ver Edición 01" : "View Issue 01"}
            </Link>
            <Link href={`/${lang}/support`} className="btn-ink">
              {lang === "es" ? "Apoyar" : "Support the print run"}
            </Link>
          </div>
        </div>
      </article>
